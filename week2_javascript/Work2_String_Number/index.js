...
const compareStr = ((first_str, second_str) => {
...
const reverseAEIOU = ((strs) => {
    if (typeof (strs) != 'string') return '';
    let strsArry = Array.from(strs);
    const mother = 'aeiou';
    let cardinality = strs.match(/[aeiou]/ig).reverse();
...
const reverseInt = ((total) => {
    ...
    let reverse = Math.abs(total).toString().split('').reverse().join('');
